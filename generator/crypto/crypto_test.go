...
func TestIsInExclusionZone(t *testing.T) {
	...
	for _, address := range bannedBitcoin {
		tool.Expect(t, true, c.isInExclusionZone(int(rune(address[0]))))
	}
	// take any banned rune and + 1 it to get a valid character
	tool.Expect(t, false, c.isInExclusionZone(int(rune(bannedBitcoin[0][0]))+1))
}

func TestGenerateBicoinAddress(t *testing.T) {
	c := faker.New().Crypto()
	length := tool.IntBetween(5, 10)
	tool.Expect(t, length+1, len(c.generateBicoinAddress(length, "a", c.Faker)))
}

func TestP2PKHAddress(t *testing.T) {
	c := faker.New().Crypto()
	addr := c.P2PKHAddress()
	tool.Expect(t, true, len(addr) >= bitcoinMin)
	tool.Expect(t, true, len(addr) <= bitcoinMax)
	tool.Expect(t, true, strings.HasPrefix(addr, validBitcoinPrefix["p2pkh"]))
	for i := 0; i < len(bannedBitcoin); i++ {
		tool.Expect(t, true, !strings.Contains(addr, bannedBitcoin[i]))
	}
}

func TestP2PKHAddressWithLength(t *testing.T) {
	c := faker.New().Crypto()
	length := tool.IntBetween(26, 62)
	addr := c.P2PKHAddressWithLength(length)
	tool.Expect(t, true, len(addr) == length)
	tool.Expect(t, true, strings.HasPrefix(addr, validBitcoinPrefix["p2pkh"]))
}
...
func TestP2SHAddress(t *testing.T) {
	...
	tool.Expect(t, true, len(addr) >= bitcoinMin)
	tool.Expect(t, true, len(addr) <= bitcoinMax)
	tool.Expect(t, true, strings.HasPrefix(addr, validBitcoinPrefix["p2sh"]))
	for i := 0; i < len(bannedBitcoin); i++ {
		tool.Expect(t, true, !strings.Contains(addr, bannedBitcoin[i]))
	}
}
...
func TestBech32Address(t *testing.T) {
	...
	tool.Expect(t, true, len(addr) >= bitcoinMin)
	tool.Expect(t, true, len(addr) <= bitcoinMax)
	tool.Expect(t, true, strings.HasPrefix(addr, validBitcoinPrefix["bech32"]))
	for i := 0; i < len(bannedBitcoin); i++ {
		tool.Expect(t, true, !strings.Contains(addr, bannedBitcoin[i]))
	}
}
...
func TestEtheriumAddress(t *testing.T) {
	...
	tool.Expect(t, true, len(addr) == ethLen)
	tool.Expect(t, true, strings.HasPrefix(addr, ethPrefix))
}
	...
	for k, tc := range []TestCaseAlnum{
		{
			...
			assert: func(t *testing.T, a int, b int) {
				tool.Expect(t, true, a == int('0'))
				tool.Expect(t, true, b == int('9'))
			},
		},
		{
			...
			localInt: 1,
			assert: func(t *testing.T, a int, b int) {
				tool.Expect(t, true, a == int('A'))
				tool.Expect(t, true, b == int('Z'))
			},
		},
		{
...
func TestRandomBitcoin(t *testing.T) {
	for k, tc := range []TestCaseRandomBitcoin{
		...
		t.Run(fmt.Sprintf("case=%d/description=%s", k, tc.desc), func(t *testing.T) {
			...
			tool.Expect(t, true, strings.HasPrefix(rs, tc.expectedSubstring))
			tool.Expect(t, true, len(rs) >= bitcoinMin)
			tool.Expect(t, true, len(rs) <= bitcoinMax)
		})
	}
}
