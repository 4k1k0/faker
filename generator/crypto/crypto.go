...
func (c Crypto) generateBicoinAddress(length int, prefix string, f *faker.Faker) string {
...
func (c Crypto) P2PKHAddress() string {
	length := tool.IntBetween(bitcoinMin, bitcoinMax)
...
func (c Crypto) EtheriumAddress() string {
	...
	for i := 0; i < ethLen-2; i++ {
		asciiStart, asciiEnd := c.algorithmRange()
		val := tool.IntBetween(asciiStart, asciiEnd)
		address = append(address, string(rune(val)))
	}
	return strings.Join(address, "")
}
